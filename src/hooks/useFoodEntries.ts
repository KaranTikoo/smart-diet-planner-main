...
export const useFoodEntries = (date?: string) => {
  ...
  const addEntry = async (entry: Omit<FoodEntry, 'id' | 'user_id' | 'created_at'>) => {
    ...
    try {
      const foodEntryData: Database['public']['Tables']['food_entries']['Insert'] = {
        ...entry,
        user_id: user.id,
      };
  ...
  const updateEntry = async (id: string, updates: Partial<FoodEntry>) => {
    ...
    try {
      const foodEntryUpdates: Database['public']['Tables']['food_entries']['Update'] = updates;

      const { data, error } = await supabase
        .from('food_entries')
        .update(foodEntryUpdates)
        .eq('id', id)
        .eq('user_id', user.id)
        .select()
        .single()
