...
export const useFoodEntries = (date?: string) => {
  ...
  const addEntry = async (entry: Omit<FoodEntry, 'id' | 'user_id' | 'created_at'>) => {
    ...
    try {
      const { data, error } = await supabase
        ...
        ] as Database['public']['Tables']['food_entries']['Insert'][]) // Explicitly cast the array
        .select()
        .single()
  ...
  const updateEntry = async (id: string, updates: Partial<FoodEntry>) => {
    ...
    try {
      const { data, error } = await supabase
        .from('food_entries')
        .update(updates as Database['public']['Tables']['food_entries']['Update']) // Explicitly cast the object
        .eq('id', id)
        .eq('user_id', user.id)
        .select()
        .single()
